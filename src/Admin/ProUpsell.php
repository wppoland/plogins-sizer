...
/**
 * PRO upgrade promotion, shown ONLY on the Mezuro settings screen: a dismissible
...
{
    private const META   = 'sizer_pro_banner_dismissed';
    private const ACTION = 'sizer_dismiss_pro';
    ...
    public function enabled(): bool
    {
        ...
        if (defined('Sizer\\Pro\\VERSION')) {
            return false;
        }
        /**
         * Filters whether the Mezuro PRO promo is shown on the settings screen.
         *
         * @param bool $show Default true.
         */
        return (bool) apply_filters('sizer/show_pro_cta', true) && $this->features() !== [];
    }
    ...
    private function url(): string
    {
         ...
         *
         * @param string $url Default the Mezuro PRO page.
         */
    ...
    private function priceLabel(): string
    {
        ...
        if (! empty($d['price_from'])) {
            ...
            return sprintf(__('from %1$s%2$d/yr', 'mezuro'), $cur, (int) $d['price_from']);
        }
        return '';
    }
    ...
    public function handleDismiss(): void
    {
        if (! current_user_can('manage_woocommerce')) {
            wp_die(esc_html__('Permission denied.', 'mezuro'));
        }
        check_admin_referer(self::ACTION);
        update_user_meta(get_current_user_id(), self::META, 1);
        wp_safe_redirect(wp_get_referer() ?: admin_url('admin.php?page=sizer'));
        exit;
    }
    ...
    public function banner(): void
    {
        ...
        $name     = (string) ($this->data()['name'] ?? 'Mezuro Pro');
        ...
        ?>
        <div class="sizer-pro-banner" role="note">
            <span class="sizer-pro-banner__tag">PRO</span>
            <p class="sizer-pro-banner__text">
                <strong><?php
                /* translators: %s: PRO edition name */
                printf(esc_html__('Do more with %s', 'mezuro'), esc_html($name)); ?></strong>
                <?php if ($subtitle !== '') : ?><span class="sizer-pro-banner__sub"><?php echo esc_html($subtitle); ?></span><?php endif; ?>
            ...
            <a class="sizer-pro-banner__dismiss" href="<?php echo esc_url($this->dismissUrl()); ?>" aria-label="<?php esc_attr_e('Dismiss this notice', 'mezuro'); ?>">&times;</a>
        </div>
        <?php
    }
    ...
    public function aside(): void
    {
        ...
        <aside class="sizer-card sizer-pro-aside" aria-labelledby="sizer-pro-aside-h">
            ...
            <h2 id="sizer-pro-aside-h" class="sizer-pro-aside__heading"><?php esc_html_e('Unlock every PRO feature', 'mezuro'); ?></h2>
            <ul class="sizer-pro-aside__list">
                <?php foreach ($features as $f) : ?>
                    <li>
                        <span class="sizer-pro-aside__lock" aria-hidden="true"></span>
            ...
            <?php if ($price !== '') : ?>
                <p class="sizer-pro-aside__price"><?php echo esc_html($price); ?><?php if ($this->sellable()) : ?> · <?php esc_html_e('one licence, every PRO feature', 'mezuro'); ?><?php endif; ?></p>
            <?php endif; ?>
        </aside>
        <?php
    }
    ...
    public function cards(): void
    {
        ...
        $name     = (string) ($this->data()['name'] ?? 'Mezuro Pro');
        ?>
        <section class="sizer-pro-cards" aria-labelledby="sizer-pro-cards-h">
            <h2 id="sizer-pro-cards-h" class="sizer-pro-cards__title">
                <?php
                /* translators: %s: PRO edition name */
                printf(esc_html__('What %s adds', 'mezuro'), esc_html($name)); ?>
            </h2>
            <div class="sizer-pro-cards__grid">
                <?php foreach ($features as $f) : ?>
                    <article class="sizer-pro-card">
                        <span class="sizer-pro-card__badge">PRO</span>
                        <span class="sizer-pro-card__lock" aria-hidden="true"></span>
                        <h3 class="sizer-pro-card__title"><?php echo esc_html($f['title']); ?></h3>
                        <?php if ($f['desc'] !== '') : ?>
                            <p class="sizer-pro-card__desc"><?php echo esc_html($f['desc']); ?></p>
                        <?php endif; ?>
                    </article>
                <?php endforeach; ?>
            </div>
